...
public class VideoThumbnailPickerVM extends BaseVM {
    public File video, pickedThumbnail;
    ...
    public LoadDataUiTask.OnExecute handleThumbnailOnExe() {
        return new LoadDataUiTask.OnExecute() {
            @Override public void onExecute(LoadDataUiTask task) {
                try {
                    task.notifyStart("正在处理...");
                    List<File> thumbnailList = VideoUtils.videoToBitmapListToTempFile(video, 20, new VideoUtils.OnVideoToBitmapListener() {
    ...
    @Override public Class<? extends IView> getViewClass() {
        return VideoThumbnailPickerView.class;
    }
}