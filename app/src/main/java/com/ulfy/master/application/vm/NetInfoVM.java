...
import com.ulfy.master.infrastructure.http.AppHttpUtils;
import com.ulfy.master.ui.view.NetInfoView;

public class NetInfoVM extends BaseVM {
    public Movie movie;
    ...
    public LoadDataUiTask.OnExecute loadMovieInfoOnExe() {
        return new LoadDataUiTask.OnExecute() {
            @Override public void onExecute(LoadDataUiTask task) {
                try {
                    task.notifyStart("正在加载...");
                    movie = AppHttpUtils.getMovieInfo(1);
                    task.notifySuccess("加载完成");
                } catch (Exception e) {
                    LogUtils.log("加载失败", e);
                    task.notifyFail(e);
