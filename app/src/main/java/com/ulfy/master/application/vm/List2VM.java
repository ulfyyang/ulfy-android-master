...
import com.ulfy.master.ui.view.List2View;

import java.util.ArrayList;
import java.util.List;

public class List2VM extends BaseVM {
    public List<List2CM> contentCMList = new ArrayList<>();              // 存放列表数据的模型列表
    // 用于跟踪分页数据的信息，如果不需要分页可以删除该成员
    public LoadListPageUiTask.LoadListPageUiTaskInfo<List2CM> contentTaskInfo = new LoadListPageUiTask.LoadListPageUiTaskInfo<>(contentCMList);
    ...
    public LoadListPageUiTask.OnLoadListPage loadContentDataPerPageOnExe() {
        ...
        return new LoadListPageUiTask.OnLoadSimpleListPage() {
            @Override protected void loadSimplePage(LoadListPageUiTask task, List<Object> modelList, List<Object> tempList, int page, int pageSize) throws Exception {
                ...
                // 正常的按照分页来了加载数据
                for (int i = 0; i < 30; i++) {
                    tempList.add(new List2CM((page - 1) * 30 + i));
                }
            }
        };
    }
